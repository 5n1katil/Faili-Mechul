#!/usr/bin/env node
/** Dark silhouette on black → white RGBA mask PNG for CustomAvatar. */
"use strict";
const fs = require("fs");
...
function decodePng(buf) {
  ...
  if (colorType !== 6 && colorType !== 2) throw new Error("Need RGB/RGBA or indexed PNG");
  const spp = colorType === 6 ? 4 : 3;
...
function packChunk(type, data) {
  ...
  data.copy(buf, 8);
  buf.writeUInt32BE(crc32(Buffer.concat([Buffer.from(type, "ascii"), data])), 8 + len);
  return buf;
}
...
if (!input || !output) {
  console.error("Usage: node boost-dark-silhouette.js <input.png> <output.png>");
  process.exit(1);
}
...
for (let i = 0; i < width * height; i++) {
  ...
  const norm = Math.min(1, (L - FLOOR) / (252 - FLOOR));
  const a = Math.min(255, Math.round(Math.pow(norm, 0.42) * 255));
  out[p] = 255;
  out[p + 1] = 255;
  out[p + 2] = 255;
  out[p + 3] = a;
}
fs.writeFileSync(output, encodePngRgba(width, height, out));
