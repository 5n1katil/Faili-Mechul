#!/usr/bin/env node
/**
 * Canva / export PNG → Hacer formatı: RGB siyah (0,0,0), silüet yalnızca alpha kanalında.
 * Hacer ve Sabri ile aynı maske davranışı için zorunlu.
 */
"use strict";
...
/** Alpha kanalını aynen koru; RGB = 0 (Hacer / Canva şeffaf export). */
...
if (!input || !output) {
  console.error("Usage: node to-hacer-format.js <input.png> <output.png>");
  process.exit(1);
}

const { width, height, rgba } = decodePng(fs.readFileSync(input));
fs.writeFileSync(output, encodePngRgba(width, height, toHacerRgba(width, height, rgba)));
console.log("Wrote", output, `${width}x${height}`, "(Hacer RGBA format)");
