#!/usr/bin/env node
/**
 * Filled white-on-black art → line-art white-on-black (same format as Tarık / Sabri).
 * Keeps outer contours + dark interior details; removes solid white fill interiors.
 */
"use strict";
...
function decodePng(buf) {
  ...
  if (!spp) throw new Error("Need RGB/RGBA PNG");
...
function outlineFilled(width, height, rgba) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      for (const [dx, dy] of neighbors) {
        ...
        if (L[ny * width + nx] < DARK) {
          edge = true;
          break;
        }
      }
      if (edge && l >= LIGHT - 40) keep[i] = 1;
    }
  }

  // Thicken lines slightly for small grid cells
  const thick = new Uint8Array(keep);
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      ...
      if (near && L[i] >= LIGHT - 60) thick[i] = 1;
    }
  }
...
function main() {
  ...
  if (!input || !output) {
    console.error("Usage: node outline-filled.js <input.png> <output.png>");
    process.exit(1);
  }
  const buf = fs.readFileSync(input);
  const { width, height, rgba } = decodePng(buf);
  const rgb = outlineFilled(width, height, rgba);
  fs.writeFileSync(output, encodePngRgb(width, height, rgb));
